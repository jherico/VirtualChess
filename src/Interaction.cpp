...
vec3 getTranslation(const spnav_event_motion & m, float scale = 1.0f / 5000.0f) {
  return vec3(m.x, m.y, -m.z) * scale;
}

quat getRotation(const spnav_event_motion & m, float scale = 1.0f / 3000.0f) {
  return quat(vec3(m.rx, m.ry, m.rz) * scale);
}
#endif
...
void translateCamera(mat4 & camera, const vec3 & delta) {
  // SAY("Translating %01.3f %01.3f %01.3f", delta.x, delta.y, delta.z);
  // Bring the vector into camera space coordinates
  vec3 eyeDelta = quat(camera) * delta;
  camera = glm::translate(mat4(), eyeDelta) * camera;
}

void rotateCamera(mat4 & camera, const quat & rot) {
  camera = camera * glm::mat4_cast(rot);
}


void recompose(mat4 & camera) {
  glm::vec4 t4 = camera[3];
  t4 /= t4.w;
  camera = glm::mat4_cast(glm::normalize(quat(camera)));
  camera[3] = t4;
}

#ifdef HAVE_SIXENSE
static quat getHydraOrientation(const sixenseControllerData & c) {
  // Bunny orientation
  quat q = quat(
      c.rot_quat[0],
      c.rot_quat[1],
  ...
  return quat(euler * 2.0f * 3.14159f / 360.0f);
}
#endif
...
void CameraControl::applyInteraction(mat4 & camera) {
#ifdef HAVE_SIXENSE
  ...
  if (hydraEnabled) {
    ...
    translateCamera(camera,
        vec3(left.joystick_x, right.joystick_y, -left.joystick_y)
            / 100.0f);
    rotateCamera(camera,
        glm::angleAxis(-right.joystick_x /100.0f, vec3(0, 1, 0)));
  }
#endif
  ...
  if (spnav >= 0) {
    ...
    while (0 != (eventType = spnav_poll_event(&event))) {
      ...
      if (SPNAV_EVENT_MOTION == eventType) {
        ...
        vec3 spaceTranslation = getTranslation(m);
...
//        camera = glm::rotate(camera, (float)m.rx / 200.0f, GlUtils::X_AXIS);
        ...
        vec3 yawAxis = glm::inverse(quat(camera)) * GlUtils::Y_AXIS;
...
//        quat currentRotation(camera);
//        quat inverse = glm::inverse(currentRotation);
//        camera = mat4_cast(inverse) * camera;
//        rot = quat(euler);
//        camera = mat4_cast(rot) * camera;
//        camera = mat4_cast(spaceRotation * currentRotation) * camera;
//        rotateCamera(camera, quat(rotation));
      }
    }
  }
//  int spnav_sensitivity(double sens);
  ...
  if (glfwJoystickPresent(0)) {
    ...
    if (x52present) {
      ...
      vec3 euler = joystick->getCalibratedVector(
          STICK_Y, STICK_Z, STICK_X);
      rotation = quat(euler / 20.0f);
    } else {
...
//
//  quat leftQuat = getHydraOrientation(left);
//  mat4 leftOrient = mat4_cast(leftQuat);
//  mat4 cameraOrient = mat4_cast(glm::inverse(camera.getOrientation()));
//  leftOrient = cameraOrient * leftOrient;
//  curOrient = quat(leftOrient);
//
//  leftQuat = camera.getOrientation() * curOrient;
//euler.y = -euler.y;
//  SAY("roll %03.2f, pitch %03.2f, yaw %03.2f", euler.z, euler.x, euler.y);
//  vec3 euler = glm::eulerAngles();
